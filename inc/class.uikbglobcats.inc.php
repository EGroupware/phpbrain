	...
	* option) any later version.                                               *
	...
	class uikbglobcats
	{
		...
		function uikbglobcats()
		{
			...
			$this->bo			= CreateObject('phpbrain.bokbglobcats');
		...
		function index()
		{
			$link_data = array
			(
				'menuaction'	=> 'phpbrain.uikbglobcats.add',
			...
			if ($_POST['done'])
			{
				$GLOBALS['phpgw']->redirect_link('/phpbrain/index.php');
			}

			$this->template->set_file(array('cat_list_t' => 'listglobcats.tpl'));
			...
			$link_data['menuaction'] = 'phpbrain.uikbglobcats.index';
			...
			for ($i=0;$i<count($categories);$i++)
			{
				...
				if ($this->acl_add_sub)
				{
					$link_data['menuaction'] = 'phpbrain.uikbglobcats.add';
				...
				{
					$link_data['menuaction'] = 'phpbrain.uikbglobcats.edit';
				...
				{
					$link_data['menuaction'] = 'phpbrain.uikbglobcats.delete';
				...
				$data = unserialize($categories[$i]['data']);
				$icon = $data['icon'];
		...
		function add()
		{
			...
			$link_data = array
			(
				'menuaction'	=> 'phpbrain.uikbglobcats.index',
				'appname'		=> $this->appname
			);
			...
			$this->template->set_file(array('form' => 'globcats_form.tpl'));
			...
			$this->template->set_var('img_options', $options);
			$this->template->set_var('img_src', '');
		...
		function edit()
		{
			...
			$this->template->set_var('img_options', $options);
			$this->template->set_var('img_src', $img);
