<?php
	...
	* option) any later version.                                               *
	...
	class uikbcategories
	{
		...
		var $images = array(1 => 'about.png', 'home.png', 'logout.png', 'nonav.png', 'select.png', 'tux.gif');
		...
		function uikbcategories()
		{
			$cats_app			= get_var('cats_app',array('GET','POST'));

			$this->bo			= CreateObject('phpbrain.bokbcategories',$cats_app);
		...
		function cat_data($edata,$data)
		{
			...
			{
				$td_data .= "<td><img src='". $GLOBALS['phpgw_info']['server']['webserver_url'] . SEP . 'phpgwapi' . SEP . 'images' . SEP . $data[$edata[$j]] . "'></td>" . "\n";
			}
			return $td_data;
		}
		...
		function index()
		{
			...
			$link_data = array
			(
				'menuaction'  => 'phpbrain.uikbcategories.index',
				'cats_app'    => $cats_app,
				'extra'       => $extra,
			...
			$GLOBALS['phpgw']->template->set_var('doneurl',$GLOBALS['phpgw']->link('/phpbrain/index.php'));
...
// -------------------------- end header declaration --------------------------------------
			...
			for ($i=0;$i<count($cats);$i++)
			{
				...
				if ($cats_level || ($level == 0))
				{
					...
					{
						$link_data['menuaction'] = 'phpbrain.uikbcategories.add';
				...
				if ($cats[$i]['owner'] == $this->account && $cats[$i]['app_name'] != 'phpgw')
				{
					$link_data['menuaction'] = 'phpbrain.uikbcategories.edit';
					...
					$link_data['menuaction'] = 'phpbrain.uikbcategories.delete';
		...
		function add()
		{
			...
			$link_data = array
			(
				'menuaction'  => 'phpbrain.uikbcategories.add',
				'cats_app'    => $cats_app,
				'extra'       => $extra,
				'global_cats' => $global_cats,
				'cats_level'  => $cats_level
			...
			$GLOBALS['phpgw_info']['flags']['java_script_thirst'] = "<script>function show_image(icon) { document.icon_img.src='" . $dir_img . "' + icon; }</script>";
			$GLOBALS['phpgw']->common->phpgw_header();
			echo parse_navbar();
			...
			if ($extra)
			{
				...
				for($i=0;$i<count($edata);$i++)
				{
					$options = '<option value=0 selected>'. lang('none') . '</option>';
					foreach ($this->images as $image)
					{
						$options .= "<option value='$image'>$image</option>";
					}
					$GLOBALS['phpgw']->template->set_var('td_data', "<select name='cat_data[icon]' onChange='javascript:show_image(this.value);'>$options</select>&nbsp;&nbsp;<img id='icon_img' name='icon_img' src='". $img  ."'>");
					$GLOBALS['phpgw']->template->set_var('lang_data',lang($edata[$i]));
					$GLOBALS['phpgw']->template->fp('row','data_row',True);
				}
			}

			$link_data['menuaction'] = 'phpbrain.uikbcategories.index';
		...
		function edit()
		{
			...
			if ($extra)
			{
				...
				for($i=0;$i<count($edata);$i++)
				{
					...
					$options .= '>' . lang('none') . '</option>';
					foreach ($this->images as $image)
					{
						$options .= "<option value='$image'";
						if ($image == $icon) $options .= ' selected';
